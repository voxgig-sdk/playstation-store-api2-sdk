...
class ContainerEntity
  ...
  def make
    opts = @_entopts.dup
    ContainerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PlaystationStoreApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Container, Hash] the current Container data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PlaystationStoreApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Container fields)
  def match_get
  ...
  #
  # @param reqmatch [ContainerLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Container.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Container, Hash] the loaded Container; raises PlaystationStoreApi2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PlaystationStoreApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
