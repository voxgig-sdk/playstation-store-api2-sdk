...
require_relative 'PlaystationStoreApi2_types'


class PlaystationStoreApi2SDK
  ...
  def initialize(options = {})
    ...
    utility = PlaystationStoreApi2Utility.new
    @_utility = utility
    ...
    feature_opts = PlaystationStoreApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PlaystationStoreApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PlaystationStoreApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PlaystationStoreApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PlaystationStoreApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PlaystationStoreApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PlaystationStoreApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = PlaystationStoreApi2Spec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PlaystationStoreApi2Error.new(
        "#{op}_allow",
        "PlaystationStoreApi2SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PlaystationStoreApi2Error => err
    ...
    if fetched.is_a?(Hash)
      status = PlaystationStoreApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PlaystationStoreApi2Error.new(
        "graphql_error", "PlaystationStoreApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PlaystationStoreApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
