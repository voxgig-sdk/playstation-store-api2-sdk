...
// PlaystationStoreApi2 SDK feature test
//
...
class FtCtrl extends PlaystationStoreApi2Control
{
...
class FtClient
{
    ...
    private PlaystationStoreApi2Utility $utility;

    public function __construct(PlaystationStoreApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): PlaystationStoreApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PlaystationStoreApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PlaystationStoreApi2Context $rootctx;

    // True when this SDK was generated with the named feature.
    public static function hasFeature(string $name): bool
    {
        if (self::$feature_config === null) {
            $config = PlaystationStoreApi2Config::shared_config();
            $f = $config['feature'] ?? [];
    ...
    public function __construct(
        ...
        $utility = new PlaystationStoreApi2Utility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (PlaystationStoreApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PlaystationStoreApi2Config::make_feature($name);
    ...
    private static function buildUrl(PlaystationStoreApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PlaystationStoreApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PlaystationStoreApi2Response($fetched);
        }
    ...
    private function fail(PlaystationStoreApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PlaystationStoreApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
