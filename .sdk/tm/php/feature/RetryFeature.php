...
// PlaystationStoreApi2 SDK retry feature
...
class PlaystationStoreApi2RetryFeature extends PlaystationStoreApi2BaseFeature
{
    ...
    public function init(PlaystationStoreApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PlaystationStoreApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PlaystationStoreApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PlaystationStoreApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
