...
module PlaystationStoreApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PlaystationStoreApi2Response.new({})
    ...
    if fetch_err
      ...
      response = PlaystationStoreApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PlaystationStoreApi2Response.new(fetched)
    else
