...
//
// NewPlaystationStoreApi2SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.PlaystationStoreApi2SDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewPlaystationStoreApi2SDK(opts)
}
...
func fcCandidates(client *sdk.PlaystationStoreApi2SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.PlaystationStoreApi2SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.PlaystationStoreApi2SDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.PlaystationStoreApi2Error
