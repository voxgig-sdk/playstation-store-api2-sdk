...
describe("ContainerEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Container(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PLAYSTATION_STORE_API2_TEST_CONTAINER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local container_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.container")))
    local container_ref01_data = nil
    if #container_ref01_data_raw > 0 then
...
function container_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/container/ContainerTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read container test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("PLAYSTATION_STORE_API2_TEST_CONTAINER_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["PLAYSTATION_STORE_API2_TEST_CONTAINER_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["PLAYSTATION_STORE_API2_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["PLAYSTATION_STORE_API2_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
