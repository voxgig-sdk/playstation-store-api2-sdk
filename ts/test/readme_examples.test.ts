...
//     (\`new PlaystationStoreApi2SDK(...)\` / \`PlaystationStoreApi2SDK.test(...)\` become
//     \`PlaystationStoreApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PlaystationStoreApi2SDK'
...
const TEST_SEED = {"entity":{"container":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PlaystationStoreApi2SDK(...)\` and any
// \`PlaystationStoreApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PlaystationStoreApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PlaystationStoreApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
