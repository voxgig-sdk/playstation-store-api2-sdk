...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*PlaystationStoreApi2SDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *PlaystationStoreApi2Error {
	return NewPlaystationStoreApi2Error(code, msg, ctx)
}
